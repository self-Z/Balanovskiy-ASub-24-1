...
float spindleAlongPosition = 0.0f;      // Первая степень - вдоль станка (ось Z)
float spindleAcrossPosition = 0.0f;     // Вторая степень - поперек станка (плоскость XZ под углом к барабану)
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Меш 1: Первая степень подвижности - линейное движение шпиндельной головы ВДОЛЬ станка (ось Z)
        // Cube.001: начальная позиция Z[-2.829:-0.830], двигаем ВПРАВО к барабану (в сторону +Z)
        // Барабан находится на Z~1.11, поэтому двигаем от текущей позиции (-1.83) к +Z
        if (ourModel.meshes.size() > 1) {
            glm::mat4 mesh1Model = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, spindleAlongPosition));
            ourModel.meshes[1].DrawWithModel(shader, mesh1Model);
        }

        // Меш 2: Вторая степень подвижности - линейное движение шпиндельной головы ПОПЕРЕК станка 
        // Cube.003: начальная позиция X[-0.22:0.93] Y[2.75:3.97] Z[-0.35:1.02]
        // Движение должно быть В СТОРОНУ БАРАБАНА: от текущей позиции к барабану (X~1.34, Z~1.11)
        // Двигаем в плоскости XZ под углом к барабану
        if (ourModel.meshes.size() > 2) {
            // Направление к барабану: от центра Cube.003 (0.35, 3.36, 0.33) к центру Cylinder (1.34, 2.37, 1.11)
            // Вектор направления: (1.34-0.35, 2.37-3.36, 1.11-0.33) = (0.99, -0.99, 0.78)
            // Для движения по поверхности станка используем компоненту XZ с углом ~45°
            float angle = glm::radians(45.0f);
            float acrossX = spindleAcrossPosition * cos(angle);  // Движение в сторону барабана по X
            float acrossZ = spindleAcrossPosition * sin(angle);  // Движение в сторону барабана по Z
...
void processInput(GLFWwindow* window) {
    ...
    // Первая степень подвижности: линейное движение шпиндельной головы ВДОЛЬ станка (ось Z)
    // Cube.001: начальная позиция Z[-2.829:-0.830], центр Z~-1.83
    // Движение: Q (назад/от барабана), E (вперед/к барабану) вдоль оси Z
    // Ограничение: не даем уйти за пределы станины и не проходить сквозь барабан
    // Станина имеет Z[-2.144:1.937], барабан Z~1.11
    if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) {
        spindleAlongPosition -= moveSpeed * deltaTime;
        // Ограничение: не уходим назад за пределы станины (min Z = -1.5)
        if (spindleAlongPosition < -1.5f) spindleAlongPosition = -1.5f;
    }
    if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS) {
        spindleAlongPosition += moveSpeed * deltaTime;
        // Ограничение: не уходим вперед к барабану слишком близко (max Z = 2.5)
        if (spindleAlongPosition > 2.5f) spindleAlongPosition = 2.5f;
    }
    
    // Вторая степень подвижности: линейное движение шпиндельной головы ПОПЕРЕК станка
    // Cube.003: начальная позиция X[-0.22:0.93] Y[2.75:3.97] Z[-0.35:1.02]
    // Движение должно быть В СТОРОНУ БАРАБАНА по поверхности станка
    // Движение: R (от барабана/назад), F (к барабану/вперед) под углом в плоскости XZ
    // Ограничение: не даем уйти слишком далеко от поверхности станка
    if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS) {
        spindleAcrossPosition -= moveSpeed * deltaTime;
        // Ограничение: не уходим слишком далеко от барабана
        if (spindleAcrossPosition < -1.0f) spindleAcrossPosition = -1.0f;
    }
    if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS) {
        spindleAcrossPosition += moveSpeed * deltaTime;
        // Ограничение: не уходим слишком далеко к барабану
        if (spindleAcrossPosition > 1.0f) spindleAcrossPosition = 1.0f;
    }
